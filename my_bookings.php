...
?>
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm relative overflow-hidden mb-4 cursor-pointer active:scale-[0.98] transition-all" 
         onclick="openModal('<?= $safeDate ?>', '<?= $safeTime ?>', '<?= $b['appointment_id'] ?>', '<?= $b['status'] ?>')">
        ...
        <?php if ($isConfirmed): ?>
            <div class="px-5 pb-5 border-t border-gray-50 pt-4" onclick="event.stopPropagation()"> <form action="cancel_booking.php" method="POST" class="cancel-form">
...
<div class="fixed bottom-0 left-0 right-0 max-w-md mx-auto p-4 bg-white border-t border-gray-100 z-20">
  <a href="booking_date.php" class="flex w-full items-center justify-center bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold py-4 rounded-xl transition-all">
    จองวัดซีน
  </a>
</div>
    ...
    form.addEventListener('submit', function(e) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          form.submit(); // ส่งข้อมูลไปลบจริงๆ
        }
      });
    });
  });
...
<script>
    ...
    function openModal(dateLabel, timeLabel, appId, status) {
        // 1. เติมข้อมูลลงใน Popup
        ...
        // 2. เรียกรูป QR Code จาก API (api_qrcode.php)
        const qrCodeImg = document.getElementById('modal-qrcode');
        qrCodeImg.src = `api_qrcode.php?id=${appId}`;

        // 3. ปรับสีและข้อความตามสถานะ (confirmed หรือ cancelled)
        ...
        // 4. แสดง Popup (Fade in และเด้งขึ้น)
    ...
    // ฟังก์ชันปิด Popup
    function closeModal() {
        // 1. ซ่อน Popup (Fade out และเด้งลง)
        modal.classList.add('opacity-0', 'pointer-events-none');
        modalContent.classList.add('translate-y-full');

        // 2. เคลียร์รูป QR Code (เพื่อป้องกันรูปเก่าค้างตอนเปิดใหม่)
        setTimeout(() => {
